...
## Check if repo is in a tag, to install this specific PS2 Dev environment
if git describe --exact-match --tags $(git log -n1 --pretty='%h') >/dev/null 2>&1; then
  TAG=$(git describe --exact-match --tags $(git log -n1 --pretty='%h'))
  echo "Installing specific version $TAG"
else
...
if [ "$1" ]; then
  ...
  for STEP in "$@"; do
    "${BUILD_SCRIPTS[$STEP-1]}" "$TAG" || { echo "${BUILD_SCRIPTS[$STEP-1]}: Failed."; exit 1; }
  done
  ...
  for SCRIPT in ${BUILD_SCRIPTS[@]}; do
    "$SCRIPT" "$TAG" || { echo "$SCRIPT: Failed."; exit 1; }
  done
